...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        printf("usage: otp_dec textFile encryptKey portNumber\n");
        exit(1);
    }
    ...
    //make sure the text contains only valid characters
    while (read(textfd, buffer, 1) != 0) {
        if (buffer[0] != ' ' && (buffer[0] < 'A' || buffer[0] > 'Z')) {
            ...
            if (buffer[0] != '\n') {
                perror("text contains invalid characters");
                exit(1);
            }
        }
    }
    //check if the key is long enough
    text_len = lseek(textfd, 0, SEEK_END);
    ...
    if (strcmp(buffer, "dec_d_jh") != 0) {
        perror("unable to contact otp_dec_d on given port");
        exit(2);
    }
    
    //send the text
    sendFile(textfd, sockfd);
    //send the key
    sendFile(keyfd, sockfd);
    
    //receive the text from the daemon and print to stdout
